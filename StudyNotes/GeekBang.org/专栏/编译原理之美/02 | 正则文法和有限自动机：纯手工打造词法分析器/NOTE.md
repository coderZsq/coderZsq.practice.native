上一讲，我提到词法分析的工作是将一个长长的字符串识别出一个个的单词，这一个个单词就是 example.sq.Token。而且词法分析的工作是一边读取一边识别字符串的，不是把字符串都读到内存再识别。你在听一位朋友讲话的时候，其实也是同样的过程，一边听，一边提取信息。

那么问题来了，字符串是一连串的字符形成的，怎么把它断开成一个个的 example.sq.Token 呢？分割的依据是什么呢？本节课，我会通过讲解正则表达式（Regular Expression）和有限自动机的知识带你解决这个问题。
...
我们来描述一下标识符、比较操作符和数字字面量这三种 example.sq.Token 的词法规则。
...
2. 标识符状态：在初始状态时，当第一个字符是字母的时候，迁移到状态 2。当后续字符是字母和数字时，保留在状态 2。如果不是，就离开状态 2，写下该 example.sq.Token，回到初始状态。
...
这里我想补充一下，你能看到上图中的圆圈有单线的也有双线的。双线的意思是这个状态已经是一个合法的 example.sq.Token 了，单线的意思是这个状态还是临时状态。
...
if (isAlpha(ch)) {              //第一个字符是字母
    ...
    token.type = example.sq.TokenType.Identifier;
    tokenText.append(ch);
} else if (isDigit(ch)) {       //第一个字符是数字
    newState = DfaState.IntLiteral;
    token.type = example.sq.TokenType.IntLiteral;
    tokenText.append(ch);
} else if (ch == '>') {         //第一个字符是>
    newState = DfaState.GT;
    token.type = example.sq.TokenType.GT;
    tokenText.append(ch);
}
```
...
其中 example.sq.Token 是自定义的一个数据结构，它有两个主要的属性：一个是“type”，就是 example.sq.Token 的类型，它用的也是一个枚举类型的值；一个是“text”，也就是这个 example.sq.Token 的文本值。
...
case GT:
    if (ch == '=') {
        token.type = example.sq.TokenType.GE;  //转换成GE
        state = DfaState.GE;
        tokenText.append(ch);
    } else {
...
示例程序的输出如下，其中第一列是 example.sq.Token 的类型，第二列是 example.sq.Token 的文本值：
...
上面的例子虽然简单，但其实已经讲清楚了词法原理，就是依据构造好的有限自动机，在不同的状态中迁移，从而解析出 example.sq.Token 来。你只要再扩展这个有限自动机，增加里面的状态和迁移路线，就可以逐步实现一个完整的词法分析器了。
...
上面的例子涉及了 4 种 example.sq.Token，这 4 种 example.sq.Token 用正则表达式表达，是下面的样子：
...
case Id_int3:
    if (isBlank(ch)) {
        token.type = example.sq.TokenType.Int;
        state = initToken(ch);
    }
    else{
        state = DfaState.Id;    //切换回Id状态
