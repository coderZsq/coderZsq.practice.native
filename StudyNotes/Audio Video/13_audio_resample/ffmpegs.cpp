...
void FFmpegs::resampleAudio(const char *inFilename,
    ...
    // 缓冲区的样本数量
    int outSamlples = av_rescale_rnd(outSampleRate, inSamlples, inSampleRate, AV_ROUND_UP);
    /*
    ...
    */
    ...
    qDebug() << "输入缓冲区" << inSampleRate << inSamlples;
    qDebug() << "输出缓冲区" << outSampleRate << outSamlples;
