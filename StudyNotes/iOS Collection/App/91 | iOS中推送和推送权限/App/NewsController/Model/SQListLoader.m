...
- (void)loadListDataWithFinishBlock:(SQListLoaderFinishBlock)finishBlock {
    ...
    NSArray<SQListItem *> *listdata = [self _readDataFromLocal];
    if (listdata) {
...
- (NSArray<SQListItem *> *)_readDataFromLocal {
    NSArray *pathArray = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
    NSString *cachePath = [pathArray firstObject];
    NSString *listDataPath = [cachePath stringByAppendingPathComponent:@"SQData/list"];
    ...
    if ([unarchiveObj isKindOfClass:[NSArray class]] && [unarchiveObj count] > 0) {
        return (NSArray<SQListItem *> *)unarchiveObj;
    }
    return nil;
}
...
//    id unarchiveObj = [NSKeyedUnarchiver unarchivedObjectOfClasses:[NSSet setWithObjects:[NSArray class], [SQListItem class], nil] fromData:testListdata error:nil];
